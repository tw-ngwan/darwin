...
class MainPage extends Component {
    ...
    handleSubmit(event) {
        ...
        axios.post(`${UPLOAD_URL}`, {
            ...
            this.setState({ ...this.state, result: response.data.result, spinCount: 0 });
        }).catch(error => {
            console.log("ERROR AT GETTING RESULTS")
            console.log(error);
            this.setState({ spinCount: 0, result: 'An error has occurred!' })
        })
    }
        ...
        return (
            <div style={{display: "flex", alignItems: "center", justifyContent: "center", height: "100vh"}}>
                ...
                <form onSubmit={this.handleSubmit}>
                    <h1 className="center-container impt label">Darwin (File Analyser)</h1>
                    <div className="center-container">
                        <label htmlFor="sentence" className="regular label font-size-24 center-container">
                            Enter the file path of the file you wish to summarize: 
                        </label>
                        <input
                    ...
                    <div className="center-container">
                        {this.state.spinCount == 1 && (
                            <h2 className="regular impt">Evaluating. This may take a while...</h2>
                        )}
                    </div>
                    <div className="center-container regular font-size-16">
                        {this.state.result}
                    </div>
                    {/* <div className="center-container">
